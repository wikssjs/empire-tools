#!/usr/bin/env python3
"""Update faq_templates in all 8 QC V3 config JSON files with niche-specific content."""
import json, os
...
FAQS = {
    ...
    'config_beton.json': [
        ...
        {'q': 'Quand peut-on couler du beton au Quebec ?',
         'a': 'La saison de coulage optimale est d\'avril a octobre, quand les temperatures restent au-dessus de 5°C jour et nuit. En dessous de ce seuil, des additifs antigel et des couvertures isolantes sont necessaires, ce qui augmente les couts. Evitez les coulages en periode de pluie ou de chaleur intense (+30°C).'},
...
for filename, faqs in FAQS.items():
    ...
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    data['content']['faq_templates'] = faqs
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    updated.append(filename)
    print(f'Updated: {filename} ({len(faqs)} FAQs)')

if errors:
    for e in errors:
        print(f'ERROR: {e}')

print(f'\nDone: {len(updated)} files updated, {len(errors)} errors.')
